...
import { Box, Paper, Typography, Divider, Collapse, IconButton } from '@mui/material';
  ...
  const renderFlowTrace = (trace: AgentTrace, index: number) => {
    ...
    return (
      ...
      >
            ...
            <Box sx={{ mt: 2, ml: 2 }}>
              {trace.flow.map((flowItem, flowIndex) => (
                ...
                >
                  ...
                  <JsonField label="Input" value={flowItem.input} fieldId={`flow-input-${flowItem.id || flowIndex}`} />
                  <JsonField label="Output" value={flowItem.output} fieldId={`flow-output-${flowItem.id || flowIndex}`} />
        ...
        {!trace.flow && (
          <>
            <JsonField label="Input" value={trace.input} fieldId={`input-${traceId}`} />
            <JsonField label="Output" value={trace.output} fieldId={`output-${traceId}`} />
          </>
        )}
      </Paper>
    );
